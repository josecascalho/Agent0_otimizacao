...
class Player(GameObject):
    name = "player"
    def __init__(self, name, number, x, y, direction, view_type, config, width=64, height=64):
        super().__init__(name, "hospital"+str(number), config, x, y, direction, width=width, height=height)
        """
        The object player has now a number. That number will be usefull to select the desired
        object to move. 
        """
...
class Goal(GameObject):
    ...
    def __init__(self, name, x, y, config):
        super().__init__(name, "house", config, x, y, "south")

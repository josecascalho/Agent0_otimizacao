...
class GameBoard(tk.Frame):
    ...
    def is_target_goal(self, coordinates):
        """Test if in the coordinates there is an obstacle"""
        return any(isinstance(obj, go.Goal) for obj in self.object_matrix[coordinates[0]][coordinates[1]])
    ...
    def view_players(self):
        ...
        for column in self.object_matrix:
            for square in column:
                for player in square:
                    if isinstance(player,go.Player):
                        players.append((player.get_number(),(player.get_x(),player.get_y())))
        return players
    ...
    def view_targets(self):
        ...
        for column in self.object_matrix:
            for square in column:
                for trg in square:
                    if isinstance(trg,go.Goal):
                        targets.append((trg.get_x(),trg.get_y()))
        return targets
